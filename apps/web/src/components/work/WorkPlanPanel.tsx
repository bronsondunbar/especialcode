...
export function WorkPlanPanel({
  ...
  const content = (
    <>
      <div className="flex items-start justify-between gap-3">
        {!inline && <DialogTitle>{data?.item.title ?? "Task"}</DialogTitle>}
        <Button variant="ghost" onClick={onClose} disabled={pending}>
          {inline ? "Hide agent controls" : "Close"}
        </Button>
      </div>
      {(error || result.error) && (
            ...
            >
              ...
              {!data.agents.length && (
                <p className="text-sm text-muted-foreground">
                  ...
                  Codex, Claude, OpenCode and Antigravity; Cursor and Grok are unavailable for this
                  workflow.
                </p>
              )}
              {!generating && (
  ...
  if (inline) return <div className="grid min-w-0 gap-3">{content}</div>;
  return (
    <Dialog
      open
      onOpenChange={(open) => {
