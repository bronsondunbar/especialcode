    ...
    >
      <DialogPopup className="max-w-3xl overflow-y-auto p-6" showCloseButton={false}>
        <div className="flex items-start justify-between gap-3">
          <DialogTitle>Plan{data ? `: ${data.item.title}` : ""}</DialogTitle>
        ...
        {!data ? (
          <p className="mt-4">Loading plan…</p>
        ) : (
          <div className="mt-5 flex flex-col gap-4">
            ...
            {!data.agents.length && (
              <p className="text-sm text-muted-foreground">
                ...
                Claude, OpenCode and Antigravity; Codex, Cursor and Grok are unavailable for this
                workflow.
              </p>
            )}
            {!generating && (
            ...
            !["ready", "awaiting_approval", "planning"].includes(data.item.status) ? (
              <p className="text-sm text-muted-foreground">
                Assign a project and move this WorkItem to Ready to generate a plan.
              </p>
            ) : null}
            {generating ? (
              ...
              plan?.content && (
                <section className="grid gap-4 rounded-xl border p-4">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Inspected files: {plan.inspectedFiles.join(", ") || "No readable source files"}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    <Button
                      variant="outline"
                      disabled={pending || data.item.status === "running"}
                      onClick={() => setEditor({ revision: plan.revision, content: plan.content! })}
                    >
                      Edit plan
                    </Button>
                    {plan.status === "draft" && (
