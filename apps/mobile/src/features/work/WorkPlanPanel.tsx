    ...
    <Modal visible presentationStyle="pageSheet" onRequestClose={onClose}>
      <SafeAreaView className="flex-1 bg-background" edges={["top", "bottom"]}>
        <ScrollView keyboardShouldPersistTaps="handled" contentContainerClassName="gap-4 p-4 pb-10">
          <ControlPill label="Close task" onPress={onClose} />
          <Text className="text-xl font-semibold">{data?.item.title ?? "Task"}</Text>
          {(error || result.error) && (
          ...
          {!data ? (
            ...
            <>
              ...
              <View
                style={visibleTab === "plan" ? undefined : { display: "none" }}
                className="gap-4"
              >
                ...
                {!data.agents.length && (
                  <Text className="text-sm text-muted-foreground">
                    Enable a supported provider in Settings. Protected planning supports Codex,
                    Claude, OpenCode and Antigravity. Cursor and Grok are unavailable for this
                    workflow.
                  </Text>
                )}
                {!generating && !canPlan && (
